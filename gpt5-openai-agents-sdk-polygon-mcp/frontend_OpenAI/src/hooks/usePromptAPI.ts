...
export function usePromptAPI(): UsePromptAPIResult {
   ...
   */
  const fetchTemplates = useCallback(async (forceRefresh = false): Promise<void> => {
    // Check cache first (unless force refresh)
    const now = Date.now();
    if (!forceRefresh && templateCache.data && (now - templateCache.timestamp) < templateCache.ttl) {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}${PROMPT_API_ENDPOINTS.TEMPLATES}`, {
        method: 'GET',
        headers: {
      ...
      const validTemplates = data.templates.filter((template): template is PromptTemplate => {
        if (!isValidPromptTemplate(template)) {
          console.warn('Invalid template received:', template);
          return false;
        }
        return true;
      });
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error fetching templates';
   ...
   */
  const generatePrompt = useCallback(async (templateId: string, ticker?: string): Promise<string> => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}${PROMPT_API_ENDPOINTS.GENERATE}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
