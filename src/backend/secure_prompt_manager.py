...
from .dynamic_prompts import UserPreferences
...
class SecureDynamicPromptManager(AdvancedPromptManager):
    ...
    def __init__(
        self, base_template: str, config: Dict[str, Any], security_config: SecurityConfig = None
    ):
    ...
    def generate_prompt(
        ...
        try:
            ...
            if not is_valid:
                ...
                response["security_info"]["rate_limited"] = any(
                    "rate limit" in error.lower() for error in security_errors
                )
            ...
            prompt, preferences = self.security_manager.circuit_breaker.call(
    ...
    def update_security_config(self, new_config: SecurityConfig, admin_user_id: str):
        ...
        self.security_manager.audit_logger.log_security_event(
            ...
            blocked=False,
            metadata={"old_config": old_config.__dict__, "new_config": new_config.__dict__},
        )
    ...
    def get_security_recommendations(self) -> Dict[str, Any]:
        ...
        if security_summary:
            ...
            if total_events > 100:
                recommendations.append(
                    "High security event volume detected. Consider reviewing rate limits."
                )
            ...
            if events_by_severity.get("critical", 0) > 0:
                recommendations.append(
                    "Critical security events detected. Immediate review required."
                )

            if events_by_severity.get("high", 0) > 10:
                recommendations.append(
                    "Multiple high-severity events. Consider tightening security."
                )
        ...
        if blocked_count > 10:
            recommendations.append(
                f"{blocked_count} users currently blocked. Review rate limiting policies."
            )
