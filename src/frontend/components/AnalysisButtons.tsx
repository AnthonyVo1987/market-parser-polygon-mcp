...
const AnalysisButtons: React.FC<AnalysisButtonProps> = ({
    ...
    const getButtonStyle = (buttonType: AnalysisButtonType): string => {
        const state = buttonStates[buttonType];
        const baseStyle =
            'px-4 py-2 rounded-lg font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed';

        if (state.loading)
            return `${baseStyle} bg-blue-100 text-blue-700 cursor-wait`;
