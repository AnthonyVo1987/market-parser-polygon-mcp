...
export function getPlaceholderText(options: PlaceholderTextOptions = {}): string {
    const {
        context = 'general',
        userState = 'idle',
        isRequired = false,
    ...
    switch (context) {
        case 'chat':
            baseText = PLACEHOLDER_TEXTS[`CHAT_${userState.toUpperCase()}` as keyof typeof PLACEHOLDER_TEXTS];
            break;
        case 'ticker':
            baseText = PLACEHOLDER_TEXTS[`TICKER_${userState.toUpperCase()}` as keyof typeof PLACEHOLDER_TEXTS];
            break;
        case 'email':
            baseText = PLACEHOLDER_TEXTS[`EMAIL_${userState.toUpperCase()}` as keyof typeof PLACEHOLDER_TEXTS];
            break;
        case 'url':
            baseText = PLACEHOLDER_TEXTS[`URL_${userState.toUpperCase()}` as keyof typeof PLACEHOLDER_TEXTS];
            break;
        default:
            baseText = PLACEHOLDER_TEXTS[`GENERAL_${userState.toUpperCase()}` as keyof typeof PLACEHOLDER_TEXTS];
    }
...
export function getChatPlaceholder(userState: PlaceholderTextOptions['userState'] = 'idle'): string {
    return getPlaceholderText({ context: 'chat', userState });
}

export function getTickerPlaceholder(userState: PlaceholderTextOptions['userState'] = 'idle'): string {
...
export function getEmailPlaceholder(userState: PlaceholderTextOptions['userState'] = 'idle'): string {
    return getPlaceholderText({ context: 'email', userState });
}

export function getUrlPlaceholder(userState: PlaceholderTextOptions['userState'] = 'idle'): string {
    return getPlaceholderText({
        context: 'url',
        userState,
...
export function validatePlaceholderText(text: string): {
    ...
    if (!text.includes('...') && !text.includes('Enter') && !text.includes('Type')) {
...
export function getPlaceholderAriaLabel(placeholder: string, fieldName: string): string {
