...
# Persistent Session 16 Test Prompts Script
# Tests all 16 standardized test prompts sequentially in a SINGLE CLI session
...
# Extended with 5 OHLC bar tests (multi-ticker, options, custom range, specific date, previous close)
...
echo -e "${CYAN}🧪 Persistent Session 16 Test Prompts Validation${NC}"
echo -e "${CYAN}================================================${NC}"
echo -e "Timestamp: $(date)"
echo -e "Max Response Time: ${MAX_RESPONSE_TIME}s per test"
echo -e "Output file: $OUTPUT_FILE"
echo -e "CLI Command: $CLI_CMD"
echo -e "Session Mode: ${GREEN}PERSISTENT${NC} (all 16 tests in same session)"
echo ""

# The 16 standardized test prompts (7 original + 4 TA indicators + 5 new OHLC/multi-ticker/options)
declare -a prompts=(
...
echo -e "${CYAN}Session will run all 16 tests sequentially${NC}"
echo ""
...
if [ $failed_tests -eq 0 ]; then
    echo -e "${GREEN}🎉 All $total_tests tests passed (including 4 TA indicators + 5 new OHLC/multi-ticker/options)!${NC}"
